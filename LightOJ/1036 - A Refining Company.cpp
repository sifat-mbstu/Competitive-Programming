...
int fun(int pos, int pre)
{
    ...
    for(int i=pre; i<=n; i++)
    {
        int now = ccm1[i][pos-1]-ccm1[pre][pos-1];
        Max = max(Max,now+cm2[i][pos]+fun(pos+1,i));
    }
    return Max;
}
int main()
