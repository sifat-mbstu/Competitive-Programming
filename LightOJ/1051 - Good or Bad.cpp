...
bool bad = 0, good = 0;
int dp[52][52][52];
int fun(int pos, int v, int c)
{
    if(v>=3 || c>=5) {bad=1; return 0;}
    ...
    if(s[pos]=='?')
    {
        ret1 = fun(pos+1,v+1,0);
        ret2 = fun(pos+1,0,c+1);
    }
    else if(is_vowel(s[pos])) ret1 = fun(pos+1,v+1,0);
    else ret1 = fun(pos+1,0,c+1);
    return rs = ret1|ret2;
}
int main()
{
