...
int sx,sy;
int dp[65536][17];
int his(int x1, int y1, int x2, int y2)
{
    int r1 = abs(x1-x2);
    int r2 = abs(y1-y2);
    return max(r1,r2);
...
int fun(int msk, int pre)
{
    if(msk == (1<<idx)-1)
    {
        if(idx==0)return 0;
//      cout <<msk << endl;
        return his(ax[pre-1], ay[pre-1], sx,sy);
    }
    int &res = dp[msk][pre];
    if(res!=-1)
        return res;
    ...
    for(int i=0; i<idx; i++)
    {
        ...
        {
            if(pre!=0)
                res = min(res,fun(bitOn(msk,i),i+1) + his(ax[i],ay[i],ax[pre-1],ay[pre-1]));
            else
                res = min(res,fun(bitOn(msk,i),i+1) + his(ax[i],ay[i],sx,sy));
        }
    }
    return res;
}
