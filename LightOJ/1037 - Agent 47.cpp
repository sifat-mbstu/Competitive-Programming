...
int n;
int health[20], mat[20][20], arr[20], dp[1<<15];
int guli(int now, int val)
{
    if(now==0) return inf;
    int d = val/now;
    if(val%now!=0) d++;
...
int fun(int mask)
{
    ...
    for(int i=0; i<n; i++)
    {
        ...
        {
            Min = min (Min,health[i]+fun(bitOn(mask,i)));
            for(int j=0; j<n; j++)
            {
                if(bitCheck(mask,j))
                {
                    if(mat[j][i]>0)Min = min(Min,guli(mat[j][i],health[i])+fun(bitOn(mask,i)));
                }
            }
        }
    }
