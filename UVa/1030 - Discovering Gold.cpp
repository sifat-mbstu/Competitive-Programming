...
//    pf(pos);pnl();
    val=0;
    if(pos+1==n) val += (arr[pos]+fun(pos+1));
    for(int i=1; i<=6; i++)
    {
        if(pos+i<n) val += (arr[pos]+fun(pos+i));
    }
    return val;
}
...
//    if(val!=-1) return val;
    ...
    val=0;
    if(pos+1==n) val += rec(pos+1);
    for(ll i=1; i<=6; i++)
    {
        if(pos+i<n) val += rec(pos+i);
    }
    return val;
}

double fin=0.00;
void trys(ll pos, ll sum, ll koita)
{
    if(pos==n)
    {
        fin += (double)((double)sum/(double)koita);
        return;
    }

    double val=0;
    if(pos+1==n) val += trys(pos+1);
    for(ll i=1; i<=6; i++)
    {
        if(pos+i<n) val += trys(pos+i);
    }
    return val;
}
int main()
{
    ...
    ll koita = rec(0);
    pfl(koita);pnl();
    double res = (double)ret/(double)koita;
    pfc(++idx);printf("%.7lf\n",res);
    }
}
