...
{
    po2[0] = po3[0] = po5[0] = 1;
    for(int i=1; i<33; i++) po2[i] = po2[i-1]*2;
    for(int i=1; i<22; i++) po3[i] = po3[i-1]*3;
    for(int i=1; i<16; i++) po5[i] = po5[i-1]*5;
    for(int i=0; i<32; i++)
    {
        for(int j=0; j<21; j++)
        {
            for(int k=0; k<15; k++)
            {
                int sum = (int)log10(double(po2[i])) + (int)log10(double(po3[j])) + (int)log10(double(po5[k]));
                if(sum<=12)
                {
                    ll now = po2[i]*po3[j]*po5[k];
                    Set.insert(now);
                }
            }
        }
